    ...
    getDaemonCommand(runVideo)
    {
        ...
        if( this.useMock )
        {
            return [
               "nice", "--19",
               "mjpg_streamer",
               "-i", `input_raspicam.so -x 1280 -y 720 -br 70 -quality 10 -framerate 10 -cs ${this.cameraID}`,
            ...
            if ( this.cameraID < 2 )
            {
               if (runVideo === true) 
               {
                  return [
                     ...
                     "-i", `input_raspicam.so -x 1280 -y 720 -br 70 -quality 10 -framerate 10 -cs ${this.cameraID}`,
                     "-o", `output_ws.so -p ${this.wsPort}`,
                     "-o", `output_file.so -f /home/pi/videos -m video.${this.cameraID}.${d}.mjpeg`,
...
// -c ${this.sslInfo.certPath} -k ${this.sslInfo.keyPath}`
    ...
    start(runVideo)
    {
        ...
        this.daemon.command = this.getDaemonCommand(runVideo);
    ...
    restart()
    {
        ...
        return this.stop()
                ...
                .then( () =>
                {
                    return this.start(this.runVideo);
                });
    }
